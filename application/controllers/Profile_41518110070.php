...
class Profile_41518110070 extends CI_Controller {
	...
	public function profilesaya($id=''){
		$data['profile'] = $this->db->query("SELECT * FROM tbl_pelanggan_41518110070 WHERE kd_pelanggan LIKE '".$id."'")->row_array();
		// die(print_r($data));
	...
	public function tiketsaya($id=''){
		$this->getsecurity();
		$data['tiket'] = $this->db->query("SELECT * FROM tbl_order_41518110070 WHERE kd_pelanggan ='".$id."' group by kd_order")->result_array();
		// die(print_r($data));
		$this->load->view('frontend/tiketmu',$data);
	}
	public function changepassword($id=''){
		$this->load->library('form_validation');
		$pelanggan = $this->db->query("SELECT password_pelanggan FROM tbl_pelanggan_41518110070 where kd_pelanggan ='".$id."'")->row_array();
