...
class Tiket_41518110070 extends CI_Controller {
	...
	public function index(){
		...
		$data['asal'] = $this->db->query("SELECT * FROM `tbl_tujuan_41518110070` ORDER BY kota_tujuan ASC ")->result_array();
		$data['tujuan'] = $this->db->query("SELECT * FROM `tbl_tujuan_41518110070` group by kota_tujuan ORDER BY kota_tujuan ASC ")->result_array();
		$data['list'] = $this->db->query("SELECT * FROM `tbl_tujuan_41518110070` ORDER BY kota_tujuan ASC ")->result_array();
		// die(print_r($data));
	...
	public function cekjadwal($tgl='' , $asl='', $tjn=''){
		...
		$data['asal'] = $this->db->query("SELECT * FROM tbl_tujuan_41518110070
               WHERE kd_tujuan ='".$asal."'")->row_array();
		$data['jadwal'] = $this->db->query("SELECT * FROM tbl_jadwal_41518110070 LEFT JOIN tbl_bus_41518110070 on tbl_jadwal_41518110070.kd_bus = tbl_bus_41518110070.kd_bus LEFT JOIN tbl_tujuan_41518110070 on tbl_jadwal_41518110070.kd_tujuan = tbl_tujuan_41518110070.kd_tujuan WHERE tbl_jadwal_41518110070.wilayah_jadwal ='".$tujuan."' AND tbl_jadwal_41518110070.kd_asal = '".$asal."'")->result_array();
		// die(print_r($data));
		if (!empty($data['jadwal'])) {
			if ($tujuan == $data['asal']['kota_tujuan']) {
				...
				$data['kursi'][$i] = $this->db->query("SELECT count(no_kursi_order) FROM tbl_order_41518110070 WHERE kd_jadwal = '".$data['jadwal'][$i]['kd_jadwal']."' AND tgl_berangkat_order = '".$data['tanggal']."' AND asal_order = '".$asal."'")->result_array();
				};
				// die(print_r($data));
	...
	public function beforebeli($jadwal="",$asal='',$tanggal=''){
		...
		if ($this->session->userdata('username')){
			...
			$data['asal'] =  $this->db->query("SELECT * FROM tbl_tujuan_41518110070
               WHERE kd_tujuan ='".$asal."'")->row_array();
			$data['jadwal'] = $this->db->query("SELECT * FROM tbl_jadwal_41518110070 LEFT JOIN tbl_bus_41518110070 on tbl_jadwal_41518110070.kd_bus = tbl_bus_41518110070.kd_bus LEFT JOIN tbl_tujuan_41518110070 on tbl_jadwal_41518110070.kd_tujuan = tbl_tujuan_41518110070.kd_tujuan WHERE kd_jadwal ='".$id."'")->row_array();
			$data['kursi'] = $this->db->query("SELECT no_kursi_order FROM tbl_order_41518110070 WHERE kd_jadwal = '".$data['jadwal']['kd_jadwal']."' AND tgl_berangkat_order = '".$data['tanggal']."' AND asal_order = '".$asal."'")->result_array();
			// print_r($this->session->userdata());
			// die(print_r($data));
	...
	public function gettiket($value=''){
		...
		for($i=0; $i<$count; $i++) {
			...
			$this->db->insert('tbl_order_41518110070', $simpan);
		}
		redirect('tiket_41518110070/checkout/'.$getkode);
	}
	public function cekorder($id=''){
		$id = $this->input->post('kodetiket');
		$sqlcek = $this->db->query("SELECT * FROM tbl_order_41518110070 LEFT JOIN tbl_bus_41518110070 on tbl_order_41518110070.kd_bus = tbl_bus_41518110070.kd_bus LEFT JOIN tbl_jadwal_41518110070 on tbl_order_41518110070.kd_jadwal = tbl_jadwal_41518110070.kd_jadwal LEFT JOIN tbl_bank_41518110070 on tbl_order_41518110070.kd_bank = tbl_bank_41518110070.kd_bank WHERE kd_order ='".$id."'")->result_array();
		if ($sqlcek == '') {
		$data['tiket'] = $sqlcek;
	...
	public function payment($id=''){
		$this->getsecurity();
		$sqlcek = $this->db->query("SELECT * FROM tbl_order_41518110070 LEFT JOIN tbl_jadwal_41518110070 on tbl_order_41518110070.kd_jadwal = tbl_jadwal_41518110070.kd_jadwal LEFT JOIN tbl_bank_41518110070 on tbl_order_41518110070.kd_bank = tbl_bank_41518110070.kd_bank WHERE kd_order ='".$id."'")->result_array();
	...
	public function insertkonfirmasi($value=''){
		...
		if ( ! $this->upload->do_upload('userfile')){
			...
			// die(print_r($data));
			$this->db->insert('tbl_konfirmasi_41518110070', $data);
	...
	public function cetak($id=''){
		$this->getsecurity();
		$order = $id;
		$data['cetak'] = $this->db->query("SELECT * FROM tbl_order_41518110070 LEFT JOIN tbl_bus_41518110070 on tbl_order_41518110070.kd_bus = tbl_bus_41518110070.kd_bus LEFT JOIN tbl_jadwal_41518110070 on tbl_order_41518110070.kd_jadwal = tbl_jadwal_41518110070.kd_jadwal LEFT JOIN tbl_tujuan_41518110070 on tbl_jadwal_41518110070.kd_tujuan = tbl_tujuan_41518110070.kd_tujuan WHERE kd_order ='".$id."'")->result_array();

		$tiket = $this->db->query("SELECT email_pelanggan FROM tbl_pelanggan_41518110070 WHERE kd_pelanggan ='".$data['cetak'][0]['kd_pelanggan']."'")->row_array();
		die(print_r($tiket));
	}
